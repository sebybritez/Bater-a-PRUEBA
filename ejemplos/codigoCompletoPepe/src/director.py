...
from tile_classifier import TileClassifier, TileType, Floor
...
class Director:
    def __init__(self, config):
        ...
        self.charged_contact_tiles = set()

    def check_and_handle_contact_tile(self):
        """Si el robot se encuentra sobre un tile con contact = true (diffuseColor RGB: 0, 0.635, 0.635),
        se detiene y espera 10 segundos para recargar la bateria por completo."""
        robot_pos = self.robot.get_position()
        current_grid_pos = self.tile_classifier.position_to_grid(robot_pos)

        # Si ya se recargo en este tile de contacto, no volver a esperar
        if current_grid_pos in self.charged_contact_tiles:
            return False

        # Comprobar si la baldosa en la que esta parado el robot fue clasificada como CONTACT
        current_tile = self.mapper.get_tile_at_pos(current_grid_pos)
        if current_tile is None or current_tile.tile_type != TileType.CONTACT:
            return False

        # Verificar que el centro del robot este suficientemente adentro de la baldosa
        # (a menos de 4.5 cm del centro de la baldosa de 12 cm) para asegurar que el
        # MainSupervisor lo detecte dentro de su bounding box de contacto
        tile_center_x = current_grid_pos[0] * 0.12
        tile_center_y = current_grid_pos[1] * 0.12
        dist_to_center = math.hypot(robot_pos.x - tile_center_x, robot_pos.y - tile_center_y)
        if dist_to_center > 0.045:
            return False

        print(f"[pepe] Robot situado sobre tile de CONTACTO en {current_grid_pos} (dist al centro: {dist_to_center:.3f}m).")
        print(f"[pepe] Esperando 10 segundos para recargar bateria...")
        self.robot.stop()
        # 10200 ms para cumplir holgadamente los 10 segundos de simulacion requeridos por el MainSupervisor (change_battery_time = 10)
        self.robot.delay(10200)
        self.charged_contact_tiles.add(current_grid_pos)
        print(f"[pepe] Recarga completada al 100% en {current_grid_pos}. Reanudando exploracion.")
        return True
    ...
    def start(self):
        ...
        while True:
            ...
            self.process_messages()
            self.update_maps()
            self.check_and_handle_contact_tile()
